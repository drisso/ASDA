...
where $\hat{y} = X \hat{\beta}(\lambda_0)$ is the predicted value of $y_i$ based on the model fitted on the training set by setting $\lambda = \lambda_0$.

## Variance-bias tradeoff

One important equivalence is that we can write the mean squared error as the sum of the variance and the squared bias of the estimator:
$$
\text{MSE}(\hat{Y}) = \text{Var}(\hat{Y}) + \mathbb{E}[\hat{Y} - Y]^2.
$$

Hence, by choosing the value of $\lambda$ that minimizes the MSE, we are looking for the best variance-bias tradeoff.
