...
Remember that if $X$ is not full rank, $X^\top X$ is singular and we cannot compute the least square estimate.
...
One soultion is to use the $\ell_1$ norm, which is a proper norm and leads to a convex problem:
...
Of course this is much related to ridge regression, which can be obatined by substituting the constraint $|| \beta ||_2 \leq t$.
